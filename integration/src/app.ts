...
export function executableLookupBody(decision: { trajectoryId: string; stepId: string; decisionId: string }) {
  return {
...
export function supervisePlanBody(input: { workspaceId: string; managerObjective: string; externalPlanId: string; plan: unknown }) {
  return {
    actor: integrationActor(),
...
export function managerApprovalBody(decision: { trajectoryId: string; stepId: string; decisionId: string }, reviewer: { id: string; name?: string }) {
  return {
...
export function humanOverrideBody(decision: { trajectoryId: string; stepId: string; decisionId: string }, reviewer: { id: string; name?: string }, reason: string, overridePayload?: Record<string, unknown>) {
  return {
    ...executableLookupBody(decision),
    reviewerId: reviewer.id,
    reviewerName: reviewer.name,
...
export function receiptBody(input: { workspaceId: string; decision: { trajectoryId: string; stepId: string; decisionId: string }; executionAuthority: string; executablePayload?: Record<string, unknown> }) {
  return {
    actor: integrationActor(),
    workspaceId: input.workspaceId,
...
export function createApp() {
  ...
  app.post("/api/run-issueops", asyncRoute(async (_req, res) => {
    const result = await postJson<any>(`${state.issueOpsBaseUrl}/api/plans/run`, {
  ...
  app.post("/api/supervise", asyncRoute(async (_req, res) => {
    if (!state.latestIssueOpsPlan) return res.status(400).json({ error: "Run IssueOps first." });
    const result = await driftGuardPostJson<any>(`${state.driftGuardBaseUrl}/api/supervise-plan`, supervisePlanBody({
  ...
  app.post("/api/approve", asyncRoute(async (req, res) => {
    ...
    const approved = await driftGuardPostJson<any>(`${state.driftGuardBaseUrl}/api/manager-approval`, managerApprovalBody(decision, { id: state.managerId, name: state.managerName }));
  ...
  app.post("/api/human-override", asyncRoute(async (req, res) => {
    ...
    const decision = findDecision(input.decisionId);
    if (!decision) return res.status(404).json({ error: "Decision not found." });
    const override = await driftGuardPostJson<any>(`${state.driftGuardBaseUrl}/api/human-override`, humanOverrideBody(decision, { id: state.managerId, name: state.managerName }, input.reason, input.overridePayload));
  ...
  app.post("/api/apply", asyncRoute(async (req, res) => {
    ...
    const executable = await driftGuardPostJson<any>(`${state.driftGuardBaseUrl}/api/executable`, executableLookupBody(decision));
    if (!executable.executable) {
      decision.executionState = "refused";
      return res.status(400).json({ error: executable.reason ?? "DriftGuard refused execution." });
    }
    const receipt = await driftGuardPostJson<any>(`${state.driftGuardBaseUrl}/api/receipt`, receiptBody({
      workspaceId: state.workspaceId,
      decision,
