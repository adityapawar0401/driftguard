...
await assert.rejects(() => driftGuardPostJson("http://localhost:4100/api/supervise-plan", {}), /DriftGuard request failed: .*connection reset/);
const app = createApp();
...
const failedSuperviseBody = await failedSupervise.json();
...
const successfulSuperviseBody = await successfulSupervise.json();
...
const approvedBody = await approvedRoute.json();
...
const appliedBody = await appliedRoute.json();
