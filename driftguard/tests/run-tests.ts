...
import type { Actor, PlannedAction } from "../src/types.js";

const repo = new JsonStore(path.join(os.tmpdir(), `driftguard-test-${Date.now()}.json`));
const service = new DriftGuardService(repo);
const workspaceId = "test-workspace";
const integrationActor: Actor = { role: "integration_adapter", id: "integration-test" };
const workerActor: Actor = { role: "worker_agent", id: "issueops-test" };
const humanActor: Actor = { role: "human_reviewer", id: "manager" };
...
const everMindRequests: unknown[] = [];
const everMindUrls: string[] = [];
...
assert.equal((everMindRequests[0] as any).user_id, "test-user");
...
function sampleDecision(decisionId: string) {
  return {
    decisionId,
    trajectoryId: `traj-${decisionId}`,
    stepId: "step-1",
    workspaceId,
    sourceSystem: "issueops" as const,
    externalPlanId: "plan",
    externalItemId: "item",
    actionType: "customer_reply_draft" as const,
    ...
    executionAuthority: "driftguard_allow" as const,
...
function sampleMemory(memoryId: string) {
  return {
    memoryId,
    workspaceId,
    memoryLayer: "advisory_memory" as const,
    memoryType: "test_memory",
    trustLevel: "llm_inferred" as const,
    authorityRank: 30,
    writerType: "advisory_llm" as const,
    writerId: "test",
...
function sampleReceipt(receiptId: string) {
  return {
    receiptId,
    workspaceId,
    ...
    executionAuthority: "driftguard_allow" as const,
    status: "LOCAL_PLACEHOLDER_RECORDED" as const,
    message: "receipt",
    createdAt: new Date().toISOString()
  };
}
...
const everMindWriteRequests: any[] = [];
...
let validationRetryBody: any;
globalThis.fetch = (async (_url: string | URL | Request, init?: RequestInit) => {
  validationRetryCalls += 1;
  validationRetryBody = init?.body ? JSON.parse(String(init.body)) : {};
  if (validationRetryCalls === 1) return new Response("needs documented shape", { status: 422 });
  return new Response(JSON.stringify({ ok: true }), { status: 200, headers: { "content-type": "application/json" } });
}) as typeof fetch;
...
globalThis.fetch = (async () => new Response("strict bad", { status: 400 })) as typeof fetch;
...
globalThis.fetch = originalFetch;
if (originalRawEventsEnv === undefined) delete process.env.DRIFTGUARD_EVERMIND_WRITE_RAW_EVENTS;
else process.env.DRIFTGUARD_EVERMIND_WRITE_RAW_EVENTS = originalRawEventsEnv;
if (originalStrictEnv === undefined) delete process.env.EVERMIND_STRICT;
else process.env.EVERMIND_STRICT = originalStrictEnv;
...
assert.match(((await missingApproval.json()) as { error: string }).error, /Persisted decision not found/);
