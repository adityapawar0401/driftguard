...
export class DriftGuardService {
    ...
    async supervisePlan(input) {
        ...
        const decisions = await Promise.all(workerPlan.planItems.map((action, index) => this.decideAction({
            workspaceId: input.workspaceId,
            trajectoryId,
...
function summarize(decisions) {
    ...
    for (const decision of decisions) {
        if (decision.verdict === "allow")
            summary.allow += 1;
        if (decision.verdict === "allow_modified")
            summary.allowModified += 1;
        if (decision.verdict === "review_required")
            summary.reviewRequired += 1;
        if (decision.verdict === "block")
            summary.block += 1;
        if (decision.verdict === "pause_and_replan")
            summary.pauseAndReplan += 1;
