...
export function evaluateAction(input) {
    ...
    if (action.actionType === "customer_reply_draft") {
        ...
        if (unsafeCommitment || missingStopUse || claimsExternalDone) {
            if (unsafeCommitment)
                reasons.push("Reply contains an unapproved refund, replacement, compensation, legal fault, or root-cause commitment.");
            if (missingStopUse)
                reasons.push("Safety-risk reply does not advise immediate stop-use and official support.");
            if (claimsExternalDone)
                reasons.push("Local placeholder action claims an external action already happened.");
...
function authorityFor(verdict) {
    if (verdict === "allow")
        return "driftguard_allow";
    if (verdict === "allow_modified")
        return "driftguard_verified_modified";
    return "not_executable";
}
function safeReply(action, isSafety) {
    const base = [
